          ...
          {view === 'dashboard' && (
            <div className="fade-in-view">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '1.25rem' }}>
                <div className="cred-card" style={{ padding: 0 }}>
                  <div className="panel-header" style={{ background: 'transparent', borderBottom: '1px solid var(--border)' }}>
                    <PieChart size={16} style={{ color: 'var(--amber)' }}/>
                    <h3>Wealth Distribution</h3>
                  </div>
                  <div className="panel-body" style={{ height: 250, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Doughnut data={doughnutData} options={doughnutOpts}/>
                  </div>
                </div>

                <div className="cred-card" style={{ padding: 0, maxHeight: 310, overflowY: 'auto' }}>
                  <div className="panel-header" style={{ position: 'sticky', top: 0, zIndex: 10, background: 'var(--bg-card)', borderBottom: '1px solid var(--border)', backdropFilter: 'blur(10px)' }}>
                    <Building size={16} style={{ color: 'var(--blue)' }}/>
                    <h3>All Accounts & Cards</h3>
                  </div>
                  <div className="panel-body" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {/* Cash */}
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 16px', background: 'var(--green-bg)', borderRadius: 'var(--r-md)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ background: 'var(--bg-card)', padding: 6, borderRadius: 8, boxShadow: 'var(--shadow-sm)' }}><Wallet size={16} style={{ color: 'var(--green)' }}/></div>
                        <span style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--green)' }}>Cash on Hand</span>
                      </div>
                      <span style={{ fontWeight: 800, fontSize: '1.1rem', color: 'var(--green)' }}>{fmt(cash)}</span>
                    </div>
                    {/* Banks */}
                    {banks.length > 0 && <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginTop: 8 }}>Bank Accounts</div>}
                    ...
                    {/* Credit Cards */}
                    {creditCards.length > 0 && <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginTop: 8 }}>Credit Cards</div>}
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem' }}>
                      {creditCards.map(cc => (
          ...
          {view === 'credit-cards' && (
            <div className="fade-in-view">
              ...
              <div className="stat-grid" style={{ marginBottom: '1.75rem' }}>
                <StatCard icon={<CreditCard size={18}/>}   color="blue"   label="Total Limit"       value={fmt(ccLimit)}/>
                    ...
                    <div key={card.id} className={`metallic-card ${['gold', 'silver', 'platinum'][idx % 3]}`}>
                      ...
                      <div style={{ zIndex: 1, position: 'relative' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                          ...
                          <div>
                            <div style={{ fontWeight: 800, fontSize: '1.1rem', color: 'white' }}>{card.bankName}</div>
                            <div style={{ fontSize: '0.8rem', opacity: 0.8 }}>{card.cardName}</div>
                          </div>
                        </div>
                        <div style={{ fontSize: '1.2rem', letterSpacing: '4px', opacity: 0.9, fontFamily: 'monospace', marginBottom: 12 }}>**** **** **** {card.cardNumber}</div>
                      </div>

                      <div style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 10, padding: '16px', zIndex: 1, position: 'relative', marginTop: 12 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.76rem', marginBottom: 8 }}>
                          <span style={{ color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>UTILIZED: {util}%</span>
                          <span style={{ color: 'white', fontWeight: 700 }}>AVAIL: {fmt(card.limit - card.outstanding)}</span>
                        </div>
                        <div className="progress-bar-wrap" style={{ marginBottom: 12, height: 6, background: 'rgba(255,255,255,0.2)' }}>
                          <div style={{ height: '100%', background: util > 80 ? '#EF4444' : '#10B981', width: `${util}%`, borderRadius: 99 }}/>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                          <div>
                            <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.65rem', fontWeight: 700, letterSpacing: 1, marginBottom: 2 }}>LIMIT</div>
                            <div style={{ color: 'white', fontWeight: 800, fontSize: '1rem' }}>{fmt(card.limit)}</div>
                          </div>
                          <div style={{ textAlign: 'right' }}>
                            <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.65rem', fontWeight: 700, letterSpacing: 1, marginBottom: 2 }}>OUTSTANDING</div>
                            <div style={{ color: util > 80 ? '#FCA5A5' : 'white', fontWeight: 900, fontSize: '1rem' }}>{fmt(card.outstanding)}</div>
                          </div>
                        </div>
                      </div>
                    </div>
